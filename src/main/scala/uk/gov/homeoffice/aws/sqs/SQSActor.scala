...
/**
  ...
  * @param sqs Sqs AWS SQS wrapper of functionality to an instance of SQS.
  ...
  * </pre>
  */
abstract class SQSActor(val sqs: SQS, filters: (Message => Option[Message])*)(implicit listeners: Seq[ActorRef] = Seq.empty[ActorRef]) extends Actor with QueueCreation with Json with ActorLogging {
  ...
  def publishError(t: Throwable, message: Message): Message = {
    ...
    sqs publishError compact(render(
